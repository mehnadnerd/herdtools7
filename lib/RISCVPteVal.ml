...
let writable ha hd p =
  (p.ab <> 0 || ha) && (* access allowed *)
  (p.db <> 0 || hd) (* write allowed *)
...
let prot_default =
  { oa=OutputAddress.PHY "";
    valid=1; ab=1; db=1; attrs=Attrs.default; }

let default s = { prot_default with  oa=OutputAddress.PHY s; }

(* TODO brs when have U bit for PTE this should clear it *)
let of_pte s = { prot_default with  oa=OutputAddress.PTE s; }
...
and pp_ab hexa ok = pp_int_field hexa ok "ab" (fun p -> p.ab)
...
let pp_fields hexa showall p k =
  ...
  let k = pp_ab hexa showall p k in
  k
...
let add_field k v p =
  match k with
  | "ab" -> { p with ab = my_int_of_string k v }
  | "db" -> { p with db = my_int_of_string k v }
  | "valid" -> { p with valid = my_int_of_string k v }
  | _ ->
      Warn.user_error "Illegal RISCV page table entry property %s" k
...
let compare =
  ...
  let cmp =
    lex_compare (fun p1 p2 -> Misc.int_compare p1.ab p2.ab) cmp in
  let cmp =
...
let fields = ["ab";"db";"valid";]
and default_fields =
  let p = prot_default in
  let ds = [p.ab; p.db; p.valid;] in
...
let mask_ab = Int64.shift_left 1L 6
let mask_all_neg =
  Int64.lognot
       (Int64.logor
...
let orop p m =
  if is_set m mask_all_neg then None
    ...
    let p = if is_set m mask_db then { p with db=0; } else p in (* Why is db and ab opposite here? *)
    let p = if is_set m mask_ab then { p with ab=1; } else p in
    Some p
...
and andnot2 p m =
  if is_set m mask_all_neg then None
    ...
    let p = if is_set m mask_ab then { p with ab=0; } else p in
    Some p
...
and andop p m =
  ...
  let r =
    if is_set m mask_ab &&  p.ab=1;
    then Int64.logor r mask_ab else r  in
  Some r
